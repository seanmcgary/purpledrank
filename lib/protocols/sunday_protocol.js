		...
		parse_command: function(socket, data){
			...
			switch(data[0]){
				...
				case 'drop':
					...
					if(socket.active_machine === null){
						return self.send_response(socket, self.config.errors['503']);
					}

					// TODO make sure the user is authenticated
					//if(socket.authed === false){
					//	return self.send_response(socket, self.config.errors['404']);
					//}

					// check to make sure the machine is actually connected
					self.server.command_exec.machine_connected(socket.active_machine, function(err, machine){
						if(err === null && machine !== false){
				...
				case 'machine':
					...
					self.server.command_exec.machines(function(err, machines){
						if(machines.length){
							// check for big drink
							_.each(machines, function(machine){
								if(_.indexOf(machine.aliases, data[1]) >= 0){
									socket.active_machine = data[1];
									machine_found = true;
									return;
								}
							});
							...
							if(machine_found === false){
								return self.send_response(socket, self.config.errors['502']);
							} else {
								return self.send_response(socket, self.config.errors['OK']);
							}
						} else {
				...
				case 'machines': 	// return a list of connected machines
					self.server.command_exec.machines(function(err, machines){
						if(machines.length){
							...
							_.each(machines, function(machine){
								machine_string += "\n" + machine.aliases[0] + " - " + machine.long_name + ((_.indexOf(machine.aliases, socket.active_machine) >= 0) ? ' (selected)' : '' );
							});
				...
				case 'getbalance':
					...
					self.server.command_exec.get_balance(socket, function(err, user_data){
						if(err === null){
							socket.credits = user_data.credits;

							return self.send_response(socket, self.config.errors['OK'] + ' ' + socket.credits);
						} else {
							return self.send_response(socket, self.config.errors['405']);
						}
					});
					break;
			}
